...
from utils.torch_utils import torch_distributed_zero_first, torch_load

import global_var


# Each DataLoader worker is already a separate process.  OpenCV's internal
# thread pool would otherwise multiply the thread count (8 workers x N OpenCV
# threads), increasing context switching and reducing paired-image throughput.
cv2.setNumThreads(0)
...
CACHE_VERSION = 0.1
CACHE_METADATA_KEYS = {'hash', 'results', 'version'}


def _dataset_scan_warn_seconds():
    """Return the no-progress interval used by dataset filesystem diagnostics."""
    try:
        return max(float(os.getenv('LCAFNET_DATASET_SCAN_WARN_SECONDS', '15')), 1.0)
    except ValueError:
        logger.warning(
            'Invalid LCAFNET_DATASET_SCAN_WARN_SECONDS=%r; using 15 seconds.',
            os.getenv('LCAFNET_DATASET_SCAN_WARN_SECONDS'))
        return 15.0


class DatasetScanWatchdog:
    """Report a filesystem scan that has stopped making progress.

    This is deliberately diagnostic rather than destructive: a slow network,
    FUSE or NTFS request is allowed to finish, while the log identifies the
    operation and last path instead of leaving startup apparently frozen.
    """

    def __init__(self, operation):
        self.operation = operation
        self.warn_seconds = _dataset_scan_warn_seconds()
        self.current = 'not started'
    ...
    def _monitor(self):
        poll_seconds = min(self.warn_seconds, 1.0)
        while not self.stop_event.wait(poll_seconds):
            stalled_for = time.monotonic() - self.last_progress
            warnings_due = int(stalled_for // self.warn_seconds)
            if warnings_due > self.warning_count:
                self.warning_count = warnings_due
                logger.warning(
                    'Dataset filesystem timeout diagnostic: %s has made no '
                    'progress for %.1fs; current/last path: %s. If this path '
                    'is on NTFS/FUSE, move the dataset to a native Linux '
                    'filesystem or check the mount.',
                    self.operation, stalled_for, self.current)
...
def get_hash(files, prefix=''):
    """Return the legacy size hash, with progress and slow-path diagnostics.

    Normal startup no longer calls this for a compatible cache. Set
    LCAFNET_VERIFY_DATASET_CACHE=1 to opt into the expensive legacy check.
    """
    total_size = 0
    watchdog = DatasetScanWatchdog('dataset metadata/hash scan').start()
    pbar = tqdm(files, desc=f'{prefix}Checking dataset metadata', unit='file', leave=False)
    try:
        for file_path in pbar:
            watchdog.progress(file_path)
...
def cache_is_compatible(cache):
    """Accept a structurally complete cache without touching every source file."""
    return (isinstance(cache, dict) and
            cache.get('version') == CACHE_VERSION and
            'results' in cache and
            all(key in cache for key in CACHE_METADATA_KEYS))


def use_cached_labels(cache, cache_path, source_files, prefix=''):
    """Decide whether to reuse a cache; full metadata validation is opt-in."""
    if not cache_is_compatible(cache):
        logger.info(
            '%sRebuilding incompatible dataset cache %s (expected version %s).',
            prefix, cache_path, CACHE_VERSION)
        return False

    verify = os.getenv('LCAFNET_VERIFY_DATASET_CACHE', '0').strip().lower()
    if verify in {'1', 'true', 'yes', 'on'}:
        logger.info('%sStrict dataset cache verification enabled for %s.', prefix, cache_path)
        return cache.get('hash') == get_hash(source_files, prefix)

    logger.info(
        '%sUsing compatible dataset cache %s directly; skipped per-file '
        'stat/hash validation. Set LCAFNET_VERIFY_DATASET_CACHE=1 to force it.',
        prefix, cache_path)
    return True
...
def create_dataloader_rgb_ir(path1, path2,  imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    sampler = sampler or (torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None)
    # A mutable AFSS sampler requires a fresh worker view each epoch so mosaic
    # augmentation sees the current subset as its auxiliary-image pool.
    # A standard DataLoader recreates worker-side dataset copies each epoch,
    # which is required when the training loop disables augmentation late in
    # training. InfiniteDataLoader keeps stale worker copies alive forever.
    loader = (torch.utils.data.DataLoader
              if image_weights or sampler is not None or mutable_augmentations
...
def load_or_rebuild_cache(cache_path, rebuild, prefix=''):
    if cache_path.is_file():
        try:
            return torch_load(cache_path), True
        except Exception as e:
            logger.warning(f'{prefix}Ignoring invalid cache {cache_path}: {e}. Rebuilding cache.')
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        # add 保存带有边框的图像
        #f = path.split('.')[0] + 'RGB.' + path.split('.')[-1]
        #cv2.imwrite(f, img)
...
#     return t
def img2label_paths(img_paths):
    """Map supported RGB/IR image layouts to their shared YOLO labels."""
    t = []
    for x in img_paths:
        path = Path(x)
        ...
        elif 'visible' in parts:
            parts[parts.index('visible')] = 'labels'
        elif 'infrared' in parts:
            parts[parts.index('infrared')] = 'labels'
        else:
            raise ValueError(f'Unknown image path: {x}')
        t.append(str(Path(*parts).with_suffix('.txt')))
    return t


def image_files_from_source(path):
    files = []
    for p in path if isinstance(path, list) else [path]:
        p = Path(p)
        if p.is_dir():
            files += scan_image_directory(p)
        elif p.is_file():
    ...
    return sorted(x.replace('/', os.sep) for x in files if x.split('.')[-1].lower() in img_formats)


def label_names_from_image_source(path):
    return sorted(Path(x).name for x in img2label_paths(image_files_from_source(path)))


def scan_image_directory(path, prefix=''):
    """Discover images recursively while showing progress and stall diagnostics."""
    path = Path(path)
    files = []
    operation = f'recursive image discovery in {path}'
    watchdog = DatasetScanWatchdog(operation).start(path)
    pbar = tqdm(desc=f'{prefix}Discovering images in {path}', unit='file', leave=False)
    try:
        for file_path in glob.iglob(str(path / '**' / '*.*'), recursive=True):
            watchdog.progress(file_path)
            pbar.update(1)
            if file_path.split('.')[-1].lower() in img_formats:
                files.append(file_path)
    finally:
        pbar.close()
        watchdog.close()
    logger.info('%sDiscovered %d images in %s.', prefix, len(files), path)
    return files
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    f += scan_image_directory(p, prefix)
        ...
        cache, exists = load_or_rebuild_cache(cache_path, lambda: self.cache_labels(cache_path, prefix), prefix)
        if exists and not use_cached_labels(
                cache, cache_path, self.label_files + self.img_files, prefix):
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        watchdog = DatasetScanWatchdog('image/label cache rebuild').start(path)
        pbar = tqdm(zip(self.img_files, self.label_files), desc='Scanning images', total=len(self.img_files))
        try:
            for i, (im_file, lb_file) in enumerate(pbar):
                watchdog.progress(f'image={im_file}, label={lb_file}')
                try:
                    # verify images
                    ...
                    if os.path.isfile(lb_file):
                        ...
                        if len(l):
                            # MFAD appends one dataset metadata field to every
                            # YOLO row. It is not a detection target.
                            if l.shape[1] == 6:
                                l = l[:, :5]
        ...
        x['hash'] = get_hash(self.label_files + self.img_files, prefix)
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p_rgb in path_rgb if isinstance(path_rgb, list) else [path_rgb]:
                ...
                if p_rgb.is_dir():  # dir
                    f_rgb += scan_image_directory(p_rgb, prefix)
                    # f = list(p.rglob('**/*.*'))  # pathlib
            ...
            for p_ir in path_ir if isinstance(path_ir, list) else [path_ir]:
                ...
                if p_ir.is_dir():  # dir
                    f_ir += scan_image_directory(p_ir, prefix)
                    # f = list(p.rglob('**/*.*'))  # pathlib
            ...
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
            assert self.img_files_rgb and self.img_files_ir, f'{prefix}No images found'
            rgb_names = [Path(x).stem for x in self.img_files_rgb]
            ir_names = [Path(x).stem for x in self.img_files_ir]
            if rgb_names != ir_names:
                rgb_only = sorted(set(rgb_names) - set(ir_names))[:5]
                ir_only = sorted(set(ir_names) - set(rgb_names))[:5]
                raise ValueError(
                    f'{prefix}RGB/IR pairs are not aligned by filename: '
                    f'{len(rgb_names)} RGB vs {len(ir_names)} IR; '
                    f'RGB-only examples={rgb_only}, IR-only examples={ir_only}')
        except Exception as e:
            raise Exception(f'{prefix}Error loading data from {path_rgb,path_ir}: {e}\nSee {help_url}')

        # Check cache. Datasets such as MFAD share one labels/train directory
        # between visible and infrared images, so keep modality-specific image
        # caches instead of rebuilding and overwriting one train.cache file.
        ...
        if cache_rgb_path == cache_ir_path:
            cache_rgb_path = cache_rgb_path.with_name(cache_rgb_path.stem + '_rgb.cache')
            cache_ir_path = cache_ir_path.with_name(cache_ir_path.stem + '_ir.cache')

        # Check RGB cache
        cache_rgb, exists_rgb = load_or_rebuild_cache(
            cache_rgb_path,
            lambda: self.cache_labels(self.img_files_rgb, self.label_files_rgb, cache_rgb_path, prefix),
            prefix)
        if exists_rgb and not use_cached_labels(
                cache_rgb, cache_rgb_path,
                self.label_files_rgb + self.img_files_rgb, prefix):
            cache_rgb, exists_rgb = self.cache_labels(self.img_files_rgb, self.label_files_rgb,
                                                      cache_rgb_path, prefix), False  # re-cache

        # Check IR cache
        cache_ir, exists_ir = load_or_rebuild_cache(
            cache_ir_path,
            lambda: self.cache_labels(self.img_files_ir, self.label_files_ir, cache_ir_path, prefix),
            prefix)
        if exists_ir and not use_cached_labels(
                cache_ir, cache_ir_path,
                self.label_files_ir + self.img_files_ir, prefix):
        ...
        if exists_ir:
            d = f"Scanning IR '{cache_ir_path}' images and labels... {nf_ir} found, {nm_ir} missing, {ne_ir} empty, {nc_ir} corrupted"
    ...
    def cache_labels(self, imgfiles, labelfiles, path=Path('./labels.cache'), prefix=''):
        ...
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        watchdog = DatasetScanWatchdog('multimodal image/label cache rebuild').start(path)
        ...
        x['hash'] = get_hash(label_files + img_files, prefix)
        x['results'] = nf, nm, ne, nc, i + 1
        x['version'] = CACHE_VERSION
        torch.save(x, path)  # save for next time
        logging.info(f'{prefix}New cache created: {path}')
...
def load_mosaic_RGB_IR(self, index1, index2):
    ...
    sampling_pool = getattr(self, 'afss_indices', self.indices_rgb)
    indices = [index_rgb] + random.choices(sampling_pool, k=3)  # 3 additional image indices
