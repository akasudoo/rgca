...
from utils.torch_utils import select_device, load_classifier, time_synchronized, torch_load
...
def detect(opt):
    ...
    if webcam:
        ...
        rgb_stems = [Path(x).stem for x in dataset.files]
        ir_stems = [Path(x).stem for x in dataset2.files]
        if len(rgb_stems) != len(ir_stems):
            raise ValueError(
                f'RGB/IR image counts differ: {len(rgb_stems)} RGB versus {len(ir_stems)} IR.')
        if rgb_stems != ir_stems:
            rgb_only = sorted(set(rgb_stems) - set(ir_stems))[:5]
            ir_only = sorted(set(ir_stems) - set(rgb_stems))[:5]
            raise ValueError(
                'RGB/IR filenames are not paired in the same order. '
                f'RGB-only examples={rgb_only}, IR-only examples={ir_only}.')
        print(f'Verified {len(rgb_stems)} paired RGB/IR inputs.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='runs/train/exp_rgca_mask_gfb_uniform_lr/weights/best.pt',
                        help='model.pt path(s)')
    parser.add_argument('--source1', type=str,
                        default='/home/dell/lcp/dataset/M3FD/images/vis_test',
                        help='visible-image source')
    parser.add_argument('--source2', type=str,
                        default='/home/dell/lcp/dataset/M3FD/images/Ir_test',
                        help='infrared-image source')
    ...
    parser.add_argument('--name', default='m3fd_rgca_inference', help='save results to project/name')
